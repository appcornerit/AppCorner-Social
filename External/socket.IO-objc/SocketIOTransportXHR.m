...
- (instancetype) initWithDelegate:(id<SocketIOTransportDelegate>)delegate_
{
    self = [super init];
    if (self) {
...
- (void) checkAndStartPoll
{
    ...
    if ([_polls count] == 0) {
        ...
        for (NSString *key in _polls) {
            NSMutableDictionary *pollData = _polls[key];
            if (pollData[@"data"] == nil) {
                restart = NO;
                break;
            }
        }
...
- (void) poll:(NSString *)data retryNumber:(int)retry
{
    ...
    pollData[@"retries"] = @(retry);
    pollData[@"connection"] = conn;
    [pollData setValue:data forKey:@"data"];
    _polls[conn.description] = pollData;
...
- (void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    NSMutableDictionary *pollData = _polls[connection.description];
    NSString *data = pollData[@"data"];
    [_polls removeObjectForKey:connection.description];
    
    NSNumber *retries = pollData[@"retries"];
