...
- (instancetype) initWithDelegate:(id<SocketIODelegate>)delegate
{
    self = [super init];
    if (self) {
        _delegate = delegate;
...
//        }
//        request.SR_SSLPinnedCertificates = [[NSArray alloc] initWithArray:certificates];
//        
...
- (void) doQueue 
{
    ...
    while ([_queue count] > 0) {
        SocketIOPacket *packet = _queue[0];
        [self send:packet];
        ...
        switch (idx) {
            ...
            case 6: {
                ...
                if ([pieces count] > 0) {
                    ...
                    SocketIOCallback callbackFunction = _acks[key];
...
- (void) connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
    ...
    if ([response respondsToSelector:@selector(statusCode)]) {
        ...
        if (statusCode >= 400) {
            ...
            NSDictionary *errorInfo = @{NSLocalizedDescriptionKey: error};
...
- (void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
    ...
    if ([_delegate respondsToSelector:@selector(socketIO:onError:)]) {
        NSMutableDictionary *errorInfo = @{NSLocalizedDescriptionKey: error};
...
{ 	
    ...
    if ([_sid length] < 1 || [data count] < 4) {
        ...
        _heartbeatTimeout = [data[1] floatValue];
