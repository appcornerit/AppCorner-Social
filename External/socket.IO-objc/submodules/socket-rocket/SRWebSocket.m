...
typedef NS_OPTIONS(NSUInteger, SROpCode)  {
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if (responseCode >= 400) {
        ...
        [self _failWithError:[NSError errorWithDomain:@"org.lolrus.SocketRocket" code:2132 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode]}]];
        return;
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:@"org.lolrus.SocketRocket" code:2145 userInfo:@{NSLocalizedDescriptionKey: @"Error writing to stream"}]];
             return;
        }
