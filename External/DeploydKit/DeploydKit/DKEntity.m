...
- (BOOL)isEqual:(id)object {
  if ([object isKindOfClass:[DKEntity class]]) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p %@> %@", NSStringFromClass([DKEntity class]), self, self.entityId, self.resultMap];
}
...
- (BOOL)fblogin:(NSError **)error facebookId:(NSString*)facebookId accessToken:(NSString*)accessToken{
    (self.loginMap)[kDKEntityFacebookId] = facebookId;
    (self.loginMap)[kDKEntityFbAccessToken] = accessToken;
    return [self sendAction:@"fblogin" error:error];
}
...
- (BOOL)sendAction:(NSString*)action error:(NSError **)error {
        ...
        if (!self.isDirty &&
            !([action isEqualToString:@"login"] || [action isEqualToString:@"fblogin"] || [action isEqualToString:@"logout"])) {
            return YES;
        }
        ...
        if([action isEqualToString:@"login"] || [action isEqualToString:@"fblogin"]){
        ...
        if([action isEqualToString:@"login"] || [action isEqualToString:@"fblogin"] || [action isEqualToString:@"logout"] ){
...
- (BOOL)commitObjectResultMap:(NSDictionary *)resultMap
  ...
  else if([method isEqualToString:@"login"] || [method isEqualToString:@"fblogin"]){
