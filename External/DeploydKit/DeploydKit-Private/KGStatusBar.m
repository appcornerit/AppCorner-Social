...
@interface KGStatusBar ()
    ...
    @property (strong, nonatomic) IBOutlet DKBlurView *blurView;
@end
...
{
    [[KGStatusBar sharedView] showWithStatus:status barColor:[UIColor whiteColor] textColor:[UIColor colorWithRed:0.0/255.0 green:122.0/255.0 blue:255.0/255.0 alpha:1.0]];
    if(![KGStatusBar sharedView].show)
    {
        [KGStatusBar sharedView].show = YES;        
        [KGStatusBar performSelector:@selector(dismiss) withObject:self afterDelay:showInterval ];
    }
}
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        stringLabel.font = [UIFont fontWithName:@"Cochin-BoldItalic" size:18.0f];
