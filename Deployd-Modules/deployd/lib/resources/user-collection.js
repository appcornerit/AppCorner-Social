var validation = require('validation')
  ...
  , request = require('request'); //added for fblogin
        ...
        break;
          ...
          if(ctx.url === '/fblogin') {
              ...
              request.get({
                          ...
                          {
                          var usernameVal = 'fb'+ctx.body.facebookId;
                          var passwordVal = ctx.body.facebookId;
                          var facebookIdVal = ctx.body.facebookId;
                          
                          uc.store.first({username: usernameVal},
                                         function(err, user) {
                                         if(!err && !user)
                                         {
                                         ctx.dpd.user.post({username: usernameVal, password: passwordVal, facebookId: facebookIdVal},
                                                           function(user, err) {
                                                           if(err) return ctx.done(err);
                                                           ctx.session.set({path: uc.path, uid: user.id}).save(ctx.done);
                                                           return;
                                                           });
